...
echo "INSTANCE. TEST 1: rendering SecurityGroup and SecurityGroupRules..."
...
echo "INSTANCE. TEST 2: rendering RDS Instance..."
...
echo "INSTANCE. TEST 3: rendering ExternalSecret and ProviderConfig..."
OUTPUT=$(run_render "$TEMP_INPUT" "$COMPOSITION" "$FUNC_CONFIG" "$EXTRA_RESOURCES" "$OBSERVED_RESOURCES")
...
echo "INSTANCE. TEST 4: Checking Readiness"
OUTPUT=$(run_render "$TEMP_INPUT" "$COMPOSITION" "$FUNC_CONFIG" "$EXTRA_RESOURCES" "$OBSERVED_RESOURCES")
...
#===================== DATABASE - COMPOSITION  TESTS =====================

INPUT="$SCRIPT_DIR/../examples/database.yaml"
COMPOSITION="$SCRIPT_DIR/../apis/database-composition.yaml"
FUNC_CONFIG="$SCRIPT_DIR/../../../test/common/functions.yaml"

echo "DATABASE. TEST 1: rendering resources"
OUTPUT=$(run_render "$INPUT" "$COMPOSITION" "$FUNC_CONFIG")
assert_counts "$OUTPUT" "Grant" 1 "Database" 1

cleanup_test
#===================== USER - COMPOSITION  TESTS =====================

INPUT="$SCRIPT_DIR/../examples/user-with-role-grant.yaml"
COMPOSITION="$SCRIPT_DIR/../apis/user-composition.yaml"
FUNC_CONFIG="$SCRIPT_DIR/../../../test/common/functions.yaml"
