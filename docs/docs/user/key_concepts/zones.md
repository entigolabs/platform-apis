...
A Zone is a mechanism for implementing security isolation between different information assets and systems. Entigo Platform enables isolation at two distinct levels:
...
These components typically cost $200-400 per month per account. For large organizations managing multiple data assets across lifecycle environments (development, testing, and production), cloud provider level isolation for every asset may becomes prohibitively expensive.
...
#### Node Pools
Each zone has one or more node pools. Workloads deployed to a zone are automatically scheduled to the zone's default node pool. Users can specify an alternative node pool within the zone when needed, for example, to deploy workloads on Spot instances. 
...
## Create a Zone
To add a zone to the platform instance, use the [Zone.tenancy.entigo.com](/api/Zone) API and follow the examples in the examples tab.
