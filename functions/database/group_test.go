...
func TestDatabaseFunction(t *testing.T) {
	...
	snapshotName := fmt.Sprintf("test-db-instance-snapshot-%s", setHash)
	esName := fmt.Sprintf("test-db-es-%s", setHash)
	pcName := "test-db-providerconfig"
